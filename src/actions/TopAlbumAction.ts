...
export class TopAlbumAction extends BaseAction {
	...
	private async updateInfo(context: string) {
		...
		const response = await fetch(`https://ws.audioscrobbler.com/2.0/?method=user.gettopalbums&user=${data.lastFmUsername}&period=${data.displayPeriod}&api_key=${data.lastFmApiKey}&format=json`);
		...
		// Get top album
		const album = responseData.topalbums.album[0];
